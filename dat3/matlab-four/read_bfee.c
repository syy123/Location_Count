...
//接口子程序，nlhs:输出参数数目, plhs:指向输出参数的指针; nrhs:输入参数数目, prhs:指向输入参数的指针
	         ...
	         int nrhs, const mxArray *prhs[])
{
    ...
    //在matlab中uint8:无符号整型0--255  相当于C语言中的unsigned char
	if (!mxIsClass(prhs[0], "uint8")) {
		mexErrMsgIdAndTxt("MIMOToolbox:read_bfee_new:notBytes","Input must be a char array");
	}

	/* create a pointer to the real data in the input matrix */
	inBytes = mxGetData(prhs[0]); //获得阵列的指针

	/* create the output matrix */
	outCell = mxCreateStructMatrix(1, 1, 12, fieldnames);  //申请1行1列的结构体矩阵,12个域
