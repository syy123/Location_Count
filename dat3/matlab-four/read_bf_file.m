...
len = ftell(f);
% fprintf('the file length is: %d  ',len);
...
while cur < (len - 3 - 215)
    % Read size and code
    field_len = fread(f, 1, 'uint16', 0, 'ieee-be');   %uint16 corresponds to 2 bytes, read 2 bytes
    fprintf('the field_len is: %d  ',field_len);
    code = fread(f,1);  %read 1 byte
    cur = cur+3;
    fprintf('cur is: %d,  code is: %d\n',cur, code);
    
    % If unhandled code, skip (seek over) the record and continue
    if (code == 187) % get beamforming or phy data
        bytes = fread(f, field_len-1, 'uint8=>uint8');    %'uint8=>uint8' means read a uint8 type integer and save it in a uint8 type integer

        cur = cur + field_len - 1;

        fprintf('cur is: %d, length of bytes is: %d  \n',cur, length(bytes));
